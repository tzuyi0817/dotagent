...
完成判準：每個 Finder 都已回報，同行號且同失效機制的重複候選已收斂成 `failure_scenario` 最具體的那個，逐行 lens 已檢查過每一個 hunk，且本輪實際派出的 lens 清單已記下（步驟 8 寫進記錄的 `lenses`）。
...
```json
{
  ...
  "review_url": null,
  "payload": {},
  "lenses": ["逐行", "被移除的行為", "跨檔契約", "資料來源", "平行實作對照", "操作情境與生命週期", "執行期與互動", "宣稱 vs 實作", "專案慣例"],
  "withdrawn": [
...
`lenses` 是本輪實際派出的 lens 名稱，追加審查靠它判斷哪些 lens 從沒對整份 diff 跑過（見 [FOLLOW-UP.md](FOLLOW-UP.md)）。
