...
- 「是否有未儲存變更」用 `isEqual` 直接比陣列，而那個陣列在語意上是集合（勾選的 id）：取消再勾回同一個選項，順序就變了，離開時會誤跳捨棄確認。比對前要先正規化（例如排序）。
- 含數量的文案在 en 語系只有複數形（`'{count} items'`），數量為 1 時顯示「1 items」。vue-i18n 要寫成 `'{count} item | {count} items'`，呼叫端也要改成傳入數量的複數寫法（v9+ 用 `t(key, n)`，v8 用 `$tc(key, n)`）。
- `v-if` 把設定欄位藏起來，但它存下的舊值仍會隨表單一起存檔：設計端看不到也清不掉。只要還有值，就要讓它顯示，並讓使用者能清除。
